...
class Container:
    ...
    def register(self):
        tmsg.showinfo("register-portal", "You are in the Registration Portal.")

    def view(self):
        windObj.destroy()
        import Phase2
        Phase2.RegWindow().windows()
        Phase2.view_port().viewevent()
        Phase2.windObj.mainloop()
    ...
    def layout(self):
        ...
        gSelect.set("None")
        rObj.append(
            Radiobutton(self.fc[1], text="Female", font=fontStyles[0], bg=colorPalette[0], variable=gSelect, value=0,
                        command=lambda: self.genderSelect(gSelect.get())))
        rObj.append(
            Radiobutton(self.fc[1], text="Male", font=fontStyles[0], bg=colorPalette[0], variable=gSelect, value=1,
                        command=lambda: self.genderSelect(gSelect.get())))
        rObj.append(
            Radiobutton(self.fc[1], text="Others", font=fontStyles[0], bg=colorPalette[0], variable=gSelect, value=2,
                        command=lambda: self.genderSelect(gSelect.get())))

        terms_and_conditions = Checkbutton(self.fc[2],
                                           text="By clicking on this button you agree to our terms and conditions.",
                                           bg=colorPalette[0], font=fontStyles[0])
    ...
    def button(self):
        btn = Button(self.fc[3], text="Register", command=lambda: self.collectInfo(), bg=colorPalette[1],
                     fg=colorPalette[0], padx=208, pady=10, font="Bitter 14")
